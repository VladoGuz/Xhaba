...
function ProductCard({ id, title, price, artisan, image, isUnique }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-md border border-pink-100/50 overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 group">
      <Link to={`/product/${id}`} className="relative h-64 overflow-hidden block bg-hueso flex items-center justify-center">
        <img 
        ...
        />
        {isUnique && (
          <span className="absolute top-4 right-4 bg-gradient-to-r from-cempasuchil to-cochinilla text-white text-xs font-bold px-4 py-1.5 rounded-full shadow-md animate-pulse">
            Pieza Única
          </span>
        )}
      </Link>
      ...
      <div className="p-6 bg-gradient-to-b from-white to-orange-50/30">
        <Link to={`/product/${id}`}>
          <h3 className="text-xl font-medium text-gray-900 mb-2 group-hover:text-cochinilla transition-colors">
            {title}
          </h3>
        </Link>
